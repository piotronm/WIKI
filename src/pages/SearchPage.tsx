...
import { wasMockUsedForArticles } from "../api/articles";
import SearchBar from "../components/SearchBar";
import Filters from "../components/Filters";
import SearchResultsList from "../components/SearchResultsList";
import { fetchTags, wasMockUsedForTags } from "../api/articles"; // 👈 Updated import
...
const SearchPage = () => {
  ...
  const [usingMockTags, setUsingMockTags] = useState(false); // 👈 New state
  ...
  useEffect(() => {
    fetchTags().then((tags) => {
      ...
      setUsingMockTags(wasMockUsedForTags()); // 👈 Track if fallback used
    });
  }, []);
  ...
  return (
    <Box maxWidth="md" mx="auto" my={4} px={2}>
      ...
      <Filters
        ...
        usingMockTags={usingMockTags} // 👈 Added to props
      />
      ...
      <Footer usingFallback={wasMockUsedForArticles()} />
    </Box>
  );
};
